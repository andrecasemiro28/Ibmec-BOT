...
class ExtratoCompraDialog(ComponentDialog):
    ...
    async def solicitar_card_id_step(self, step_context: WaterfallStepContext):
        """
        Solicita o ID do cartão - CORRIGIDO para pedir ID em vez do número
        """
        ...
        prompt_message = MessageFactory.text(
            "💳 **ID do Cartão**\n\n"
            "Digite o **ID do cartão** (não o número do cartão):\n\n"
            "💡 **Exemplo**: Se for seu primeiro cartão, digite **1**\n"
            "💡 **Dica**: IDs são números sequenciais: 1, 2, 3, etc."
        )

        prompt_options = PromptOptions(
            prompt=prompt_message,
            retry_prompt=MessageFactory.text("❌ Por favor, digite um número válido para o ID do cartão (exemplo: 1, 2, 3...)."),
        )
    ...
    async def buscar_extrato_step(self, step_context: WaterfallStepContext):
        ...
        if not card_id_input.isdigit():
            await step_context.context.send_activity(
                MessageFactory.text("❌ ID do cartão deve ser um número válido (exemplo: 1, 2, 3...).")
            )
            return await step_context.end_dialog()
        ...
        # Busca o extrato
        await step_context.context.send_activity(
            MessageFactory.text(f"🔄 Buscando extrato do cartão ID {card_id} para usuário {user_id}...")
        )
            ...
            if transacoes is None:
                await step_context.context.send_activity(
                    MessageFactory.text(
                        "❌ **Erro na Consulta**\n\n"
                        "Possíveis causas:\n"
                        "• Cartão ID não encontrado\n"
                        "• Cartão não pertence ao usuário\n"
                        "• Erro temporário no sistema\n\n"
                        "🔄 **Verifique os dados e tente novamente.**\n\n"
                        "💡 **Dica**: Use o ID do cartão (1, 2, 3...) e não o número do cartão!"
                    )
                )
                return await step_context.end_dialog()
                ...
                await step_context.context.send_activity(
                    MessageFactory.text(
                        "📄 **Extrato Vazio**\n\n"
                        f"O cartão ID {card_id} ainda não possui transações registradas.\n\n"
                        "💡 **As transações aparecerão aqui quando você:**\n"
                        "• Realizar compras\n"
                        "• Adicionar créditos ao cartão\n"
                        "• Fazer outras operações financeiras\n\n"
                        "🛒 **Experimente fazer uma compra e consulte novamente!**"
                    )
                )
                return await step_context.end_dialog()
    ...
    async def exibir_transacoes(self, transacoes, step_context: WaterfallStepContext):
        ...
        resumo = (
            ...
            f"💵 **Saldo Estimado:** R$ {saldo_atual:.2f}\n\n"
        )
